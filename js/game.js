...
const ORDER_UP = [[], [], ["e", "t", "s", "sb"], ["hb", "ss"], ["sp"], []];

const LAYER_EFFS = {
	b() {
		if (tmp.hcActive ? tmp.hcActive[11] : true) return new Decimal(1);
		return Decimal.pow(
			Decimal.add(2, tmp.atbb).max(0),
			player.b.points.plus(getFreeBoosters()).times(getBoosterPower())
		).max(0);
	},
	g() {
		return Decimal.pow(
			Decimal.add(2, tmp.atgb)
				.times(tmp.sGenPowEff)
				.times(
					player.ss.upgrades.includes(23)
						? LAYER_UPGS.ss[23].currently()
						: 1
				)
				.max(0),
			player.g.points.times(getGenPow())
		)
			.sub(1)
			.times(getGenPowerGainMult())
			.max(0);
	},
	t() {
		return {
			gain: Decimal.pow(
				Decimal.add(3, tmp.attb),
				player.t.points
					.plus(
						player.t.extCapsules
							.plus(tmp.freeExtCap)
							.times(getFreeExtPow())
					)
					.times(getCapPow())
				...
				.sub(1)
				.times(getTimeEnergyGainMult()),
			limit: Decimal.pow(
				Decimal.add(2, tmp.attb),
				player.t.points
					.plus(
						player.t.extCapsules
							.plus(tmp.freeExtCap)
				...
				.sub(1)
				.times(100)
				.times(getTimeEnergyLimitMult()),
		};
	},
	sb() {
		return Decimal.pow(
			Decimal.add(1.5, addToSBBase()),
			player.sb.points.times(getSuperBoosterPow())
		);
	},
	sg() {
		return Decimal.pow(Decimal.add(2, addToSGBase()), player.sg.points)
			.sub(1)
			.times(getSuperGenPowerGainMult())
			.max(0);
	},
	h() {
		let ret = player.h.points
			.plus(1)
			.times(
				player.points
					.times(player.h.points)
					.plus(1)
					.log10()
					.plus(1)
					.log10()
			...
			.log10()
			.times(5)
			.root(player.q.upgrades.includes(12) ? 1.25 : 2);
	...
	hb() {
		return Decimal.pow(
			Decimal.add(1.6, addToHBBase()),
			player.hb.points
				.pow(getHyperBoosterExp())
				.times(getHyperBoosterPow())
		);
	},
	ss() {
		return player.ss.points.pow(2.5).times(getSubspaceGainMult());
	},
	ba() {
		...
		return {
			power: points1
				.pow(0.2)
				.pow(tmp.baExp ? tmp.baExp : 1)
				.pow(player.ba.upgrades.includes(41) ? 2 : 1)
				.times(
					player.ba.upgrades.includes(54)
						? LAYER_UPGS.ba[54].currently()
						: 1
				),
			pos: player.ba.points.pow(0.7).pow(tmp.baExp ? tmp.baExp : 1),
			neg: player.ba.points
				.pow(0.65)
				.times(0.4)
				.pow(tmp.baExp ? tmp.baExp : 1),
		};
...
const LAYER_UPGS = {
	p: {
		...
		31: {
			desc:
				"Prestige Point gain is boosted by your Prestige Point amount.",
			...
			currently() {
				let ret = player.p.points
					...
					.log10()
					.plus(1)
					.pow(player.p.points.plus(1).log10().div(200).plus(1))
					.pow(
						player.p.upgrades.includes(32)
							? LAYER_UPGS.p[32].currently()
							: 1
					);
				let capStart = new Decimal("1e1000");
				if (player.sp.upgrades.includes(32))
					capStart = capStart.times(LAYER_UPGS.sp[32].currently());
				if (ret.gte(capStart))
					ret = ret.log10().times(capStart.div(1e3));
				if (player.sp.upgrades.includes(11)) ret = ret.pow(100);
				return ret;
			},
			effDisp(x) {
		...
		32: {
			...
			currently() {
				const ret = player.points.plus(1).log10().plus(1).root(16);
				return ret;
			},
			effDisp(x) {
				return `${format(x.sub(1).times(100))}% stronger`;
			},
		},
		33: {
	...
	b: {
		...
		11: {
			...
			currently() {
				return player.b.points.sqrt().plus(1).max(1.5);
			},
			effDisp(x) {
				return `${format(x)}x`;
			},
		...
		12: {
			...
			currently() {
				return player.g.points
					...
					.div(3)
					.times(
						player.t.upgrades.includes(14) &&
							!(tmp.hcActive ? tmp.hcActive[12] : true)
							? 8.5
							: 1
					);
			},
			effDisp(x) {
				return `+${format(x)} to base`;
			},
		},
		13: {
			...
			cost: new Decimal(8),
			unl() {
				return player.b.best.gte(8);
			},
			currently() {
				return player.p.points.plus(1).log10().plus(1).log10().div(3);
			},
			effDisp(x) {
				return `+${format(x)} to base`;
			},
		...
		21: {
			...
			unl() {
				return (
					player.b.upgrades.includes(11) &&
					player.b.upgrades.includes(12)
				);
			},
		},
		22: {
		...
		23: {
			...
			unl() {
				return (
					player.b.upgrades.includes(21) ||
					player.b.upgrades.includes(22)
				);
			},
			currently() {
				return player.points
					...
					.plus(1)
					.pow(3.2)
					.pow(tmp.spaceBuildEff ? tmp.spaceBuildEff[4] : 1);
			},
			effDisp(x) {
				return `/${format(x)}`;
			},
		...
		31: {
			...
			unl() {
				return player.hb.upgrades.includes(14);
			},
			currently() {
				return Decimal.pow(4, player.hb.points.pow(2));
			},
			effDisp(x) {
				return `${format(x)}x`;
			},
	...
	g: {
		...
		11: {
			...
			currently() {
				return player.g.points.sqrt().plus(1).max(1.5);
			},
			effDisp(x) {
				return `${format(x)}x`;
			},
		...
		15: {
			...
			cost: new Decimal(15),
			unl() {
				return player.g.upgrades.includes(13);
			},
			currently() {
				let ret = player.g.points
					...
					.plus(1)
					.times(
						player.e.upgrades.includes(32) &&
							!(tmp.hcActive ? tmp.hcActive[12] : true)
							? LAYER_UPGS.e[32].currently()
							: 1
					);
		...
		21: {
			...
			cost: new Decimal(18),
			unl() {
				return player.g.upgrades.includes(15);
			},
			currently() {
				return player.g.power.plus(1).log10().plus(1);
			},
			effDisp(x) {
				return `${format(x)}x`;
			},
		...
		22: {
			...
			currently() {
				return player.p.points
					.plus(1)
					.pow(0.25)
					.pow(player.g.upgrades.includes(32) ? 2.5 : 1);
			},
			effDisp(x) {
				return `/${format(x)}`;
			},
		...
		23: {
			...
			cost: new Decimal(20),
			unl() {
				return player.b.unl && player.g.upgrades.includes(15);
			},
			currently() {
				return player.b.points.pow(0.75).plus(1);
			},
			effDisp(x) {
				return `^${format(x)}`;
			},
		...
		24: {
			...
			unl() {
				return (
					player.g.upgrades.includes(14) &&
					(player.g.upgrades.includes(21) ||
						player.g.upgrades.includes(22))
				);
			},
		},
		25: {
			...
			currently() {
				return player.p.points
					...
					.pow(
						player.g.upgrades.includes(31)
							? LAYER_UPGS.g[31].currently()
							: 1
					);
			},
			effDisp(x) {
		...
		31: {
			...
			unl() {
				return player.ss.upgrades.includes(21);
			},
			currently() {
				return player.g.points.plus(1).log10().pow(3.6).plus(1);
			},
			effDisp(x) {
				return `${format(x.sub(1).times(100))}% stronger`;
			},
		...
		33: {
			...
			currently() {
				return player.g.power.plus(1).log10().div(15).plus(1);
			},
			effDisp(x) {
				return `+${format(x)}`;
			},
		...
		35: {
			...
			currently() {
				return player.ss.subspace
					.plus(1)
					.pow(40)
					.pow(player.q.upgrades.includes(54) ? 20 : 1);
			},
			effDisp(x) {
				return `${format(x)}x`;
			},
	...
	e: {
		...
		11: {
			...
			currently() {
				let exp = 1;
				if (
					player.e.upgrades.includes(14) &&
					!(tmp.hcActive ? tmp.hcActive[12] : true)
				)
					exp = 1.5;
				return {
					g: player.b.points.plus(1).log10().pow(exp),
					b: player.g.points.plus(1).log10().pow(exp),
				};
			},
			effDisp(x) {
				return (
		...
		12: {
			...
			unl() {
				return player.e.unl && player.e.best.gte(40);
			},
			currently() {
				let ret = player.e.points
					.plus(1)
					.pow(
						player.e.upgrades.includes(15) &&
							!(tmp.hcActive ? tmp.hcActive[12] : true)
							? 3.25
							: 1.5
					);
		...
		13: {
			...
			unl() {
				return (
					player.e.upgrades.includes(11) ||
					player.e.upgrades.includes(12)
				);
			},
		},
		14: {
			...
			unl() {
				return (
					player.e.upgrades.includes(13) &&
					(player.t.unl || player.s.unl)
				);
			},
		},
		15: {
		...
		22: {
			desc:
				"This layer behaves as if you chose it first (base req is now 1e120 points)",
			cost: new Decimal(1e22),
			unl() {
				return (
					(player.t.unl && player.s.unl && player.e.order == 2) ||
					player.e.upgrades.includes(22) ||
					player.e.upgrades.includes(23)
				);
			},
		},
		23: {
		...
		24: {
			...
			unl() {
				return player.t.unl && player.s.unl && player.e.best.gte(1e50);
			},
			currently() {
				return player.p.points.plus(1).pow(0.002);
			},
			effDisp(x) {
				return `${format(x)}x`;
			},
		...
		25: {
			desc:
				"Enhancers are stronger based on your Space Energy & Time Capsules.",
			cost: new Decimal(7.777e77),
			unl() {
				return player.t.unl && player.s.unl && player.e.best.gte(1e60);
			},
			currently() {
		...
		31: {
			...
			unl() {
				return player.e.upgrades.includes(25) && player.sb.unl;
			},
			currently() {
				return player.sb.points.pow(0.75).div(4).plus(1);
			},
			effDisp(x) {
				return `${format(x.sub(1).times(100))}% stronger`;
			},
		...
		32: {
			desc:
				"Generator Upgrade 5 is stronger based on your Enhance Points.",
			cost: new Decimal(7.5e108),
			unl() {
				return player.e.upgrades.includes(25) && player.sb.unl;
			},
			currently() {
				const ret = Decimal.pow(
					10,
					player.e.points.plus(1).log10().pow(0.085)
				)
					.div(10)
					.max(1)
					.min(10);
		...
		35: {
			...
			unl() {
				return (
					player.e.upgrades.includes(33) ||
					player.e.upgrades.includes(34)
				);
			},
			currently() {
				return player.points.plus(1).pow(0.004);
			},
			effDisp(x) {
				return `${format(x)}x`;
			},
	...
	t: {
		...
		11: {
			...
			currently() {
				return player.t.points
					...
					.plus(0.5)
					.plus(
						player.t.upgrades.includes(13) &&
							!(tmp.hcActive ? tmp.hcActive[12] : true)
							? LAYER_UPGS.t[13].currently()
							: 0
					);
			},
			effDisp(x) {
		...
		12: {
			desc:
				"The Time Energy cap starts later based on your Boosters, and you get a free Extra Time Capsule.",
			cost: new Decimal(3),
			unl() {
				return player.t.best.gte(2) && player.t.unl;
			},
			currently() {
				return player.b.points
					.pow(0.95)
					.plus(1)
					.pow(player.q.upgrades.includes(43) ? 16 : 1);
			},
			effDisp(x) {
				return `${format(x)}x`;
			},
		...
		13: {
			desc:
				"Extra Time Capsules are added to the first Time Upgrade's effect formula, but at a reduced amount.",
			cost: new Decimal(3),
			unl() {
				return player.t.upgrades.includes(11);
			},
			currently() {
				return player.t.extCapsules.plus(tmp.freeExtCap).pow(0.95);
			},
			effDisp(x) {
				return `+${format(x)}`;
			},
		...
		14: {
			desc:
				"Generator Upgrades 2 & 10 are 275% stronger, and Booster Upgrade 2 is 750% stronger.",
			cost: new Decimal(4),
			unl() {
				return (
					player.t.upgrades.includes(12) ||
					player.t.upgrades.includes(13)
				);
			},
		},
		21: {
			desc:
				"Time Energy boosts its own production & limit, and the Time Energy effect uses a better formula.",
			cost: new Decimal(4),
			unl() {
				return player.t.upgrades.includes(14);
			},
			currently() {
				return player.t.energy.plus(1).log10().pow(1.1).plus(1);
			},
			effDisp(x) {
				return `${format(x)}x`;
			},
		...
		22: {
			desc:
				"Time Energy production & limit are boosted by your Enhance Points.",
			cost: new Decimal(5),
			unl() {
				return player.t.upgrades.includes(14) && player.e.unl;
			},
			currently() {
				let ret = player.e.points
					.plus(1)
					.pow(0.8 / (1 + player.t.order));
				if (ret.gte("1e400"))
					ret = ret.log10().times(Decimal.div("1e400", 400)).min(ret);
				return ret;
			},
			effDisp(x) {
				return `${format(x)}x`;
		...
		23: {
			desc:
				"Time Energy production & limit are boosted by your Space Energy.",
			cost: new Decimal(5),
			unl() {
				return player.t.upgrades.includes(14) && player.s.unl;
			},
			currently() {
				return Decimal.pow(3, player.s.points.pow(0.9));
			},
			effDisp(x) {
				return `${format(x)}x`;
			},
		...
		24: {
			...
			cost: new Decimal(7),
			unl() {
				return player.t.upgrades.includes(21) && player.t.best.gte(5);
			},
		},
		31: {
			...
			unl() {
				return (
					(player.t.upgrades.includes(22) &&
						(player.e.order == 1 || player.s.unl)) ||
					(player.t.upgrades.includes(23) &&
						(player.s.order == 1 || player.e.unl))
				);
			},
		},
		32: {
			...
			unl() {
				return (
					(player.s.unl && player.e.unl) ||
					player.t.upgrades.includes(32)
				);
			},
		},
		33: {
		...
		34: {
			desc:
				"Time Energy caps later and generates faster based on your non-free Time Capsules.",
			cost: new Decimal(18),
			unl() {
				return player.t.upgrades.includes(33) && player.sb.unl;
			},
			currently() {
				return Decimal.pow(10, player.t.points.pow(1.2));
			},
			effDisp(x) {
				return `${format(x)}x`;
			},
	...
	s: {
		...
		12: {
			...
			cost: new Decimal(3),
			unl() {
				return player.s.best.gte(2) && player.s.unl;
			},
			currently() {
				return player.g.power.plus(1).log10().plus(1);
			},
		...
		13: {
			desc:
				"Space Building Levels boost Generator Power gain, and get 2 extra Space.",
			cost: new Decimal(3),
			unl() {
			...
			currently() {
				return Decimal.pow(
					20,
					Object.values(player.s.buildings).reduce((a, b) =>
						Decimal.add(a, b)
					)
				);
			},
		...
		14: {
			desc:
				"Unlock a 4th Space Building, and add a free level to all Space Buildings.",
			cost: new Decimal(4),
			unl() {
				return (
					player.s.upgrades.includes(12) &&
					player.s.upgrades.includes(13)
				);
			},
		},
		21: {
			...
			cost: new Decimal(4),
			unl() {
				return player.s.upgrades.includes(14);
			},
			currently() {
				return player.g.points.plus(1).log10().div(1.5).plus(1);
			},
			effDisp(x) {
				return `${format(x.sub(1).times(100))}% stronger`;
			},
		...
		22: {
			...
			cost: new Decimal(6),
			unl() {
				return player.s.upgrades.includes(14) && player.t.unl;
			},
			currently() {
		...
		23: {
			...
			cost: new Decimal(5),
			unl() {
				return player.s.upgrades.includes(14) && player.e.unl;
			},
			currently() {
				return player.e.enhancers
					.sqrt()
					.div(player.s.order == 0 ? 5 : 7)
					.plus(1);
			},
			effDisp(x) {
				return `${format(x.sub(1).times(100))}% stronger`;
		...
		24: {
			desc:
				"Space Building costs scale half as fast, and you have 3 more Space.",
			cost: new Decimal(7),
			unl() {
				return (
					player.s.upgrades.includes(21) &&
					(player.t.unl || player.e.unl)
				);
			},
		},
		31: {
			...
			unl() {
				return (
					(player.s.upgrades.includes(22) &&
						(player.t.order == 0 || player.e.unl)) ||
					(player.s.upgrades.includes(23) &&
						(player.e.order == 0 || player.t.unl))
				);
			},
		},
		32: {
			...
			unl() {
				return (
					(player.s.upgrades.includes(22) &&
						(player.t.order == 1 || player.e.unl)) ||
					(player.s.upgrades.includes(23) &&
						(player.e.order == 1 || player.t.unl))
				);
			},
		},
		33: {
			...
			unl() {
				return (
					(player.t.unl && player.e.unl) ||
					player.s.upgrades.includes(33)
				);
			},
		},
		34: {
			desc:
				"Space Buildings boost the Generator Power effect (before all other boosts).",
			cost: new Decimal(15),
			unl() {
				return (
					player.t.unl &&
			...
			effDisp(x) {
				return `Add ${format(x)} to exponent`;
			},
		},
		41: {
			...
			unl() {
				return player.ba.upgrades.includes(51);
			},
			currently() {
				return Decimal.pow(
					"1e4000",
					getSpace().plus(player.s.spent).sqrt()
				);
			},
			effDisp(x) {
				return `/${format(x)}`;
	...
	sb: {
		...
		11: {
			...
			cost: new Decimal(2),
			unl() {
				return player.sb.unl;
			},
			currently() {
				let ret = Decimal.pow(
					10,
					player.p.points.plus(1).log10().div(1e5).sqrt()
				);
		...
		12: {
			...
			cost: new Decimal(4),
			unl() {
				return player.sb.upgrades.includes(11);
			},
			currently() {
				return player.g.points.div(10).pow(0.04).max(1);
			},
			effDisp(x) {
				return `${format(x)}x`;
			},
		...
		21: {
			...
			cost: new Decimal(8),
			unl() {
				return player.h.challs.includes(32);
			},
			currently() {
				return player.sb.points.pow(2.15);
			},
			effDisp(x) {
				return `+${format(x)}`;
			},
		...
		22: {
			...
			currently() {
				return player.sb.points.plus(1).log10().div(3);
			},
			effDisp(x) {
				return `+${format(x)}`;
			},
	...
	q: {
		...
		11: {
			desc:
				"Quirks & Hindrance Spirit boost Point, Prestige Point, and Enhance Point gain.",
			cost: new Decimal(1),
			unl() {
				return player.q.unl && player.q.layers.gt(0);
			},
			currently() {
				return player.q.points
					.plus(1)
					.times(player.h.points.plus(1))
					.pow(0.75);
			},
			effDisp(x) {
				return `${format(x)}x`;
		...
		12: {
			desc:
				"The Quirk Energy and Hindrance Spirit effects use better formulas.",
			cost: new Decimal(5),
			unl() {
				return player.q.upgrades.includes(11) && player.h.best.gte(3);
			},
		},
		13: {
			...
			unl() {
				return (
					player.q.upgrades.includes(11) &&
					player.h.challs.includes(11)
				);
			},
		},
		14: {
		...
		21: {
			...
			unl() {
				return (
					(player.q.upgrades.includes(12) ||
						player.q.upgrades.includes(13)) &&
					player.h.challs.includes(12)
				);
			},
			currently() {
				return player.q.points
					...
					.log10()
					.plus(1)
					.pow(player.m.upgrades.includes(42) ? 1.5 : 1);
			},
			effDisp(x) {
				return `${format(x)}x`;
			},
		...
		23: {
			...
			unl() {
				return (
					player.q.upgrades.includes(21) ||
					player.q.upgrades.includes(22)
				);
			},
		},
		24: {
			...
			currently() {
				return player.q.energy.div(1e6).plus(1).pow(0.9);
			},
			effDisp(x) {
				return `${format(x)}x`;
			},
		...
		32: {
			...
			unl() {
				return (
					player.q.upgrades.includes(23) ||
					player.q.upgrades.includes(31)
				);
			},
		},
		33: {
			...
			currently() {
				return player.t.points
					.plus(player.t.extCapsules.plus(tmp.freeExtCap))
					.plus(1)
					.log10()
					.plus(1);
			},
		...
		34: {
			...
			currently() {
				return player.e.points.plus(1).log10().cbrt().plus(1);
			},
			effDisp(x) {
				return `${format(x)}x`;
			},
		...
		44: {
			...
			currently() {
				return player.q.energy.plus(1).log10().plus(1);
			},
			effDisp(x) {
				return `${format(x)}x`;
			},
		...
		51: {
			...
			unl() {
				return player.ba.upgrades.includes(52);
			},
			currently() {
				return player.q.energy.plus(1).log10().plus(1).log10();
			},
			effDisp(x) {
				return `+${format(x)}`;
			},
		...
		52: {
			...
			currently() {
				return Decimal.pow(10, player.q.layers);
			},
			effDisp(x) {
				return `${format(x)}x`;
			},
		...
		53: {
			desc:
				"The second Enhancer effect also multiplies the Booster/Generator base.",
	...
	hb: {
		...
		11: {
			...
			cost: new Decimal(2),
			unl() {
				return player.hb.unl;
			},
			currently() {
				return player.hb.points.sqrt().div(4).plus(1);
			},
			effDisp(x) {
				return `${format(x.sub(1).times(100))}% stronger`;
			},
		...
		12: {
			...
			currently() {
				return player.sb.points.div(10).plus(1).log10().plus(1);
			},
			effDisp(x) {
				return `${format(x.sub(1).times(100))}% stronger`;
			},
		...
		13: {
			...
			unl() {
				return (
					player.hb.order > 0 ||
					player.ss.upgrades.includes(15) ||
					player.hb.upgrades.includes(13) ||
					player.m.unl ||
					player.ba.unl
				);
			},
	...
	ss: {
		...
		11: {
			...
			cost: new Decimal(1),
			unl() {
				return player.ss.unl;
			},
			currently() {
				return player.ss.points.sqrt().times(150).floor();
			},
			effDisp(x) {
				return `${formatWhole(x)} more Space`;
			},
		},
		12: {
			...
			currently() {
				return player.points.plus(1).log10().div(1e4).plus(1);
			},
			effDisp(x) {
				return `${format(x)}x`;
			},
		...
		22: {
			...
			unl() {
				return (
					player.ss.upgrades.includes(21) &&
					(player.h.challs.includes(51) ||
						player.h.challs.includes(52))
				);
			},
			currently() {
				return player.ss.subspace.plus(1).root(2.25);
			},
			effDisp(x) {
				return `${format(x)}x`;
			},
		...
		23: {
			...
			cost: new Decimal(6),
			unl() {
				return player.ba.upgrades.includes(24);
			},
			currently() {
				return player.ss.subspace
					.sub(1e20)
		...
		24: {
			...
			currently() {
				return Decimal.pow(2, player.ss.points);
			},
			effDisp(x) {
				return `${format(x)}x`;
			},
		...
		25: {
			...
			currently() {
				return Decimal.pow(10, player.q.layers.sqrt());
			},
			effDisp(x) {
				return `${format(x)}x`;
			},
	...
	m: {
		...
		12: {
			...
			cost: new Decimal(25),
			unl() {
				return player.m.upgrades.includes(11);
			},
		},
		13: {
		...
		14: {
			...
			unl() {
				return player.m.upgrades.includes(12);
			},
			currently() {
				return player.m.best.div(3).plus(1).pow(0.8);
			},
			effDisp(x) {
				return `${format(x)}x`;
			},
		...
		21: {
			...
			cost: new Decimal(1000),
			unl() {
				return player.m.upgrades.includes(13);
			},
			currently() {
		...
		22: {
			...
			unl() {
				return player.m.upgrades.includes(14);
			},
		},
		23: {
			...
			unl() {
				return player.m.upgrades.includes(21);
			},
		},
		24: {
			...
			unl() {
				return player.m.upgrades.includes(22);
			},
			currently() {
				return player.b.points.plus(1).log10().times(0.9);
			},
			effDisp(x) {
				return `+${format(x)}`;
			},
		...
		31: {
			...
			unl() {
				return player.m.upgrades.includes(23) && player.sg.unl;
			},
		},
		32: {
			...
			unl() {
				return player.m.upgrades.includes(24);
			},
			currently() {
				return player.hb.points.plus(1).pow(3);
			},
			effDisp(x) {
				return `+${formatWhole(x)}`;
			},
		...
		33: {
			...
			cost: new Decimal(2e10),
			unl() {
				return player.m.upgrades.includes(31);
			},
			currently() {
				return player.h.points
					.plus(1)
		...
		34: {
			...
			unl() {
				return player.m.upgrades.includes(32);
			},
		},
		41: {
			desc:
				"You can insert more Magic into Spells, making them stronger.",
			cost: new Decimal(2.5e14),
			unl() {
				return player.m.upgrades.includes(34);
			},
		},
		42: {
		...
		43: {
			...
			cost: new Decimal(1e15),
			unl() {
				return player.m.upgrades.includes(41);
			},
			currently() {
				return player.m.hexes.plus(1).log10().plus(1).sqrt().min(86400);
			},
			effDisp(x) {
				return `${format(x)}x`;
			},
		...
		44: {
			...
			currently() {
				return player.m.points.plus(1).log10().div(10);
			},
			effDisp(x) {
				return `+${format(x)}`;
			},
	...
	ba: {
		...
		11: {
			...
			cost: new Decimal(25),
			unl() {
				return player.ba.unl;
			},
		},
		12: {
			desc:
				"Subspace is generated faster based on your Positivity & Negativity.",
			cost: new Decimal(40),
			unl() {
				return player.ba.upgrades.includes(11);
			},
			currently() {
				return (tmp.balEff2 ? tmp.balEff2 : new Decimal(1))
					.max(1)
					.pow(4);
			},
			effDisp(x) {
		...
		13: {
			desc:
				"Multiply all Quirk Layers based on your Balance Power, and the Quirk Energy effect is cubed.",
			cost: new Decimal(50),
			unl() {
				return player.ba.upgrades.includes(11);
			},
			currently() {
				return player.ba.power.plus(1).pow(1.25);
			},
			effDisp(x) {
				return `${format(x)}x`;
			},
		...
		14: {
			...
			unl() {
				return player.ba.upgrades.includes(12);
			},
		},
		21: {
			...
			unl() {
				return player.ba.upgrades.includes(13) && player.sg.unl;
			},
			currently() {
				return player.ba.negativity.plus(1).sqrt();
			},
			effDisp(x) {
				return `${format(x)}x`;
			},
		...
		22: {
			...
			unl() {
				return player.ba.upgrades.includes(14);
			},
			currently() {
				return player.ba.power.plus(1).pow(0.15);
			},
			effDisp(x) {
				return `${format(x)}x`;
			},
		...
		23: {
			...
			unl() {
				return player.ba.upgrades.includes(21);
			},
			currently() {
				return player.sg.points.pow(2).div(2);
			},
			effDisp(x) {
				return `+${format(x)}`;
			},
		...
		24: {
			...
			unl() {
				return player.ba.upgrades.includes(22);
			},
		},
		31: {
			desc:
				"The Quirk Layer cost is adjusted based on your Balance Upgrades bought.",
			cost: new Decimal(4e5),
			unl() {
				return player.ba.upgrades.includes(23);
			},
			currently() {
				return Decimal.div(
					0.8,
					Math.pow(player.ba.upgrades.length + 1, 0.1)
				).plus(1.2);
			},
			effDisp(x) {
				return `2 -> ${format(x)}`;
		...
		33: {
			...
			unl() {
				return player.ba.upgrades.includes(31);
			},
		},
		34: {
			...
			unl() {
				return player.ba.upgrades.includes(32);
			},
		},
		41: {
			...
			unl() {
				return (
					player.ba.upgrades.includes(33) &&
					player.ba.upgrades.includes(34)
				);
			},
		},
		42: {
			desc:
				"The Space Building 1 effect is stronger based on your Space Building 1 amount.",
			...
			currently() {
				return player.s.buildings[1].plus(1).pow(0.8);
			},
			effDisp(x) {
				return `^${format(x)}`;
			},
		...
		43: {
			...
			unl() {
				return (
					player.ba.upgrades.includes(41) ||
					player.ba.upgrades.includes(42)
				);
			},
		},
		44: {
			...
			unl() {
				return (
					player.ba.upgrades.includes(42) ||
					player.ba.upgrades.includes(43)
				);
			},
		},
		51: {
			...
			unl() {
				return player.ba.upgrades.includes(43);
			},
		},
		52: {
		...
		53: {
			desc:
				"The Positivity & Negativity boost to Balance Power gain is stronger based on your Super-Generator Power.",
			cost: new Decimal(2e19),
			unl() {
				return player.ba.upgrades.includes(51) && player.sg.unl;
			},
			currently() {
				return player.sg.power.plus(1).log10().div(25).plus(1).sqrt();
			},
			effDisp(x) {
				return `^${format(x)}`;
			},
		...
		54: {
			desc:
				"Balance Power boosts the first Balance Energy effect (even stronger based on your Best Balance Power).",
			cost: new Decimal(5e25),
			unl() {
				return player.ba.upgrades.includes(53);
			},
			currently() {
				return player.ba.power
					.plus(1)
					.times(player.ba.best.plus(1).sqrt())
					.cbrt();
			},
			effDisp(x) {
				return `${format(x)}x`;
	...
	sp: {
		...
		11: {
			desc:
				"The Prestige Upgrade 3, 6, & 7 effects are raised to the power of 100.",
			cost: new Decimal(1),
			unl() {
		...
		12: {
			desc:
				"Total Super-Prestige Points boost Magic & Balance Power gain.",
			cost: new Decimal(2),
			unl() {
				return player.sp.upgrades.includes(11);
			},
			currently() {
				return player.sp.total.plus(1).pow(2.5);
			},
			effDisp(x) {
				return `${format(x)}x`;
			},
		...
		13: {
			...
			cost: new Decimal(3),
			unl() {
				return player.sp.upgrades.includes(12);
			},
		},
		14: {
			desc:
				"Your Best Super-Prestige Points boost Hex & Balance Energy gain.",
			cost: new Decimal(8),
			unl() {
				return player.sp.upgrades.includes(13);
			},
			currently() {
				return player.sp.best.plus(1).pow(1.9);
			},
			effDisp(x) {
				return `${format(x)}x`;
			},
		...
		22: {
			desc:
				'The effects of "Anti-Upgrades" & "Prestigeless" Hindrances are 24,900% stronger.',
			cost: new Decimal(20),
			unl() {
				return (
					player.sp.upgrades.includes(12) ||
					player.sp.upgrades.includes(21)
				);
			},
		},
		23: {
			desc:
				"Spells are stronger based on your Total Super-Prestige Points.",
			cost: new Decimal(30),
			unl() {
				return (
					player.sp.upgrades.includes(13) ||
					player.sp.upgrades.includes(22)
				);
			},
			currently() {
		...
		24: {
			...
			unl() {
				return (
					player.sp.upgrades.includes(14) ||
					player.sp.upgrades.includes(23)
				);
			},
			currently() {
		...
		31: {
			...
			cost: new Decimal(1000),
			unl() {
				return player.sp.upgrades.includes(22);
			},
		},
		32: {
			desc:
				"Prestige Upgrade 7 softcaps later based on your Super-Prestige Points.",
			cost: new Decimal(4000),
			unl() {
				return (
					player.sp.upgrades.includes(23) &&
					player.sp.upgrades.includes(31)
				);
			},
			currently() {
				return player.sp.points.plus(1).log10().plus(1).pow(1e4);
			},
			effDisp(x) {
				return (
					`${format(x.pow(player.sp.upgrades.includes(11) ? 100 : 1)) 
					}x later`
				);
			},
		},
		33: {
			...
			unl() {
				return (
					player.sp.upgrades.includes(24) &&
					player.sp.upgrades.includes(32)
				);
			},
			currently() {
				return player.points.plus(1).log10().pow(0.1);
			},
			effDisp(x) {
				return `${format(x)}x`;
			},
...
const TAB_REQS = {
	...
	b() {
		return (
			(player.b.unl || player.points.gte(tmp.layerReqs.b)) &&
			layerUnl("b")
		);
	},
	g() {
		return (
			(player.g.unl || player.points.gte(tmp.layerReqs.g)) &&
			layerUnl("g")
		);
	},
	e() {
		return (
			(player.e.unl || player.points.gte(tmp.layerReqs.e)) &&
			layerUnl("e")
		);
	},
	t() {
		return (
			(player.t.unl || player.points.gte(tmp.layerReqs.t)) &&
			layerUnl("t")
		);
	},
	s() {
	...
	sb() {
		return (
			(player.sb.unl || player.b.points.gte(tmp.layerReqs.sb)) &&
			layerUnl("sb")
		);
	},
	sg() {
		return (
			(player.sg.unl || player.g.points.gte(tmp.layerReqs.sg)) &&
			layerUnl("sg")
		);
	},
	h() {
		return (
			(player.h.unl || player.t.energy.gte(tmp.layerReqs.h)) &&
			layerUnl("h")
		);
	},
	q() {
		return (
			(player.q.unl || player.g.power.gte(tmp.layerReqs.q)) &&
			layerUnl("q")
		);
	},
	hb() {
		return (
			(player.hb.unl || player.sb.points.gte(tmp.layerReqs.hb)) &&
			layerUnl("hb")
		);
	},
	ss() {
		return (
			(player.ss.unl || player.s.points.gte(tmp.layerReqs.ss)) &&
			layerUnl("ss")
		);
	},
	m() {
		return (
			(player.m.unl || player.h.points.gte(tmp.layerReqs.m)) &&
			layerUnl("m")
		);
	},
	ba() {
		return (
			(player.ba.unl || player.q.points.gte(tmp.layerReqs.ba)) &&
			layerUnl("ba")
		);
	},
	sp() {
		return (
			(player.sp.unl || player.p.points.gte(tmp.layerReqs.sp)) &&
			layerUnl("sp")
		);
	},
};
...
function getLayerEffDesc(layer) {
	...
	switch (layer) {
		case "b":
			return (
				`translated to a ${format(eff)}x multiplier to point gain`
			);
			break;
		case "g":
			return (
				`which are generating ${format(eff)} Generator Power/sec`
			);
			break;
		case "t":
			return (
				...
				format(eff.gain) 
				} Time Energy/sec, but with a limit of ${ 
				format(eff.limit) 
				} Time Energy`
			);
			break;
		case "sb":
			return (
				`which are multiplying the Booster effect base by ${ 
				format(eff)}`
			);
			break;
		case "sg":
		...
		case "h":
			return (
				`which are providing ${ 
				format(eff) 
				} free extra Time Capsules (boosted by your points)`
			);
			break;
		case "hb":
			return (
				`which are multiplying the Super-Booster effect base by ${ 
				format(eff)}`
			);
			break;
		case "ss":
...
function checkForVars() {
	...
	if (player.s.buildings[4] === undefined)
		player.s.buildings[4] = new Decimal(0);
	if (player.s.buildings[5] === undefined)
		player.s.buildings[5] = new Decimal(0);
...
function convertToDecimal() {
	...
	for (let i = 1; i <= 5; i++)
		player.s.buildings[i] = new Decimal(player.s.buildings[i]);
	...
	for (let i = 1; i <= 4; i++)
		player.m.casted[i] = new Decimal(player.m.casted[i]);
...
function exponentialFormat(num, precision) {
	...
	return (
		`${m.toStringWithDecimalPlaces(3)}e${e.toStringWithDecimalPlaces(0)}`
	);
}
...
function format(decimal, precision = 3) {
	...
	if (decimal.gte("ee1000"))
		return `ee${format(decimal.log10().log10())}`;
...
function getLayerReq(layer) {
	...
	switch (layer) {
		...
		case "e":
			req = req.times(
				Decimal.pow("1e200", Decimal.pow(player.e.order, 2))
			);
			break;
		case "t":
			req = req.times(
				Decimal.pow("1e200", Decimal.pow(player.t.order, 2))
			);
			break;
		case "s":
			req = req.times(
				Decimal.pow("1e200", Decimal.pow(player.s.order, 2))
			);
			break;
		case "hb":
	...
	switch (layer) {
		case "p":
			...
			if (player.b.upgrades.includes(11))
				mult = mult.times(LAYER_UPGS.b[11].currently());
			if (player.g.upgrades.includes(11))
				mult = mult.times(LAYER_UPGS.g[11].currently());
			if (player.e.unl) mult = mult.times(tmp.enhEff);
			if (
				player.e.upgrades.includes(12) &&
				!(tmp.hcActive ? tmp.hcActive[12] : true)
			)
				mult = mult.times(LAYER_UPGS.e[12].currently());
			if (
				player.e.upgrades.includes(13) &&
				!(tmp.hcActive ? tmp.hcActive[12] : true)
			)
			...
			if (
				...
				mult = mult.times(1e40);
			if (player.t.unl) mult = mult.times(tmp.timeEff);
			if (player.s.unl && tmp.spaceBuildEff)
				mult = mult.times(tmp.spaceBuildEff[1]);
			if (player.q.upgrades.includes(11))
				mult = mult.times(LAYER_UPGS.q[11].currently());
			if (tmp.hcActive ? tmp.hcActive[62] : true) mult = mult.times(0);
			break;
		case "b":
			if (player.b.upgrades.includes(23))
				mult = mult.div(LAYER_UPGS.b[23].currently());
			if (player.s.unl && tmp.spaceBuildEff)
				mult = mult.div(tmp.spaceBuildEff[3]);
			break;
		case "g":
			if (player.g.upgrades.includes(22))
				mult = mult.div(LAYER_UPGS.g[22].currently());
			if (player.s.unl && tmp.spaceBuildEff)
				mult = mult.div(tmp.spaceBuildEff[3]);
			break;
		case "e":
			if (
				player.e.upgrades.includes(24) &&
				!(tmp.hcActive ? tmp.hcActive[12] : true)
			)
				mult = mult.times(LAYER_UPGS.e[24].currently());
			...
			break;
		case "t":
			if (player.h.challs.includes(21))
				mult = mult.div(H_CHALLS[21].currently());
			break;
		case "s":
			if (player.h.challs.includes(21))
				mult = mult.div(H_CHALLS[21].currently());
		...
		case "sg":
			...
			break;
		case "h":
			if (player.h.challs.includes(71))
				mult = mult.times(H_CHALLS[71].currently());
			if (player.q.upgrades.includes(22))
				mult = mult.times(LAYER_UPGS.q[22].currently().h);
			if (player.q.upgrades.includes(34))
				mult = mult.times(LAYER_UPGS.q[34].currently());
			if (player.q.upgrades.includes(44))
				mult = mult.times(LAYER_UPGS.q[44].currently());
			if (player.ba.unl) mult = mult.times(tmp.balEff);
			if (player.m.unl) mult = mult.times(tmp.hexEff);
			break;
		case "q":
			if (player.h.challs.includes(12))
				mult = mult.times(H_CHALLS[12].currently());
			if (player.q.upgrades.includes(22))
				mult = mult.times(LAYER_UPGS.q[22].currently().q);
			if (player.q.upgrades.includes(34))
				mult = mult.times(LAYER_UPGS.q[34].currently());
			if (player.ba.unl) mult = mult.times(tmp.balEff);
			if (player.m.unl) mult = mult.times(tmp.hexEff);
			break;
		case "m":
			if (player.sp.upgrades.includes(12))
				mult = mult.times(LAYER_UPGS.sp[12].currently());
			break;
		case "ba":
			if (player.sp.upgrades.includes(12))
				mult = mult.times(LAYER_UPGS.sp[12].currently());
			break;
		case "sp":
			if (player.sp.upgrades.includes(24))
				mult = mult.times(LAYER_UPGS.sp[24].currently());
			if (player.sp.upgrades.includes(33))
				mult = mult.times(LAYER_UPGS.sp[33].currently());
			break;
	}
	return mult;
}
...
function rowReset(row, layer) {
	const prev = JSON.parse(JSON.stringify(player)); // Deep Copy
	const start = getStartPlayer();
	switch (row) {
		case 0:
		...
		case 1:
			...
			if (
				(LAYER_ROW[layer] >= 3 && player.h.best.gte(10)) ||
				player.sp.total.gte(1)
			)
				player.p.upgrades = prev.p.upgrades;
			else if (
				(player.h.best.lt(1) && player.q.best.lt(1)) ||
				LAYER_ROW[layer] >= 3
			) {
				if (layer == "b" || layer == "g") {
		...
		case 2:
			...
			player.b.best = player.m.best.gte(1)
				? player.b.best
				: new Decimal(0);
			if (!player.t.best.gte(4) && !player.sp.total.gte(1))
				player.b.upgrades = [];
			player.g.points = new Decimal(0);
			player.g.power = new Decimal(0);
			player.g.best = player.m.best.gte(1)
				? player.g.best
				: new Decimal(0);
			if (!player.s.best.gte(4) && !player.sp.total.gte(1))
				player.g.upgrades = [];
		...
		case 3:
			...
			player.t.order = 0;
			if (player.h.best.lt(2) && player.m.best.lt(1))
				player.t.best = new Decimal(0);
			if (player.h.best.lt(4) && !player.sp.total.gte(1))
				player.t.upgrades = [];
			...
			player.s = {
				...
				points: new Decimal(0),
				best:
					player.h.best.gte(2) || player.m.best.gte(1)
						? player.s.best
						: new Decimal(0),
				spent:
					player.q.best.gte(4) &&
				...
				upgrades:
					player.h.best.gte(4) || player.sp.total.gte(1)
						? player.s.upgrades
						: [],
				auto: player.s.auto,
			...
			player.sb = {
				...
				best:
					player.h.best.gte(2) || player.m.best.gte(1)
						...
						: new Decimal(0),
				upgrades:
					player.h.best.gte(10) || player.sp.total.gte(1)
						? player.sb.upgrades
						: [],
			};
			player.sg = {
		...
		case 4:
			player.h = {
				...
				points: new Decimal(0),
				best:
					player.ba.best.gte(1) || player.m.best.gte(1)
						? player.h.best
						: new Decimal(0),
				active: 0,
				challs:
					player.m.best.gte(2) || player.sp.total.gte(1)
						? player.h.challs
						: [],
				upgrades: [],
			};
			player.q = {
				...
				time: new Decimal(0),
				upgrades:
					player.ba.best.gte(2) || player.sp.total.gte(1)
						? player.q.upgrades
						: [],
			};
			player.hb = {
				...
				best:
					player.ba.best.gte(1) || player.m.best.gte(1)
						...
						: new Decimal(0),
				upgrades:
					player.ba.best.gte(5) || player.sp.total.gte(1)
						? player.hb.upgrades
						: [],
			};
			player.ss = {
...
function doReset(layer, force = false) {
	...
	if (
		(layer == "b" && player.t.best.gte(12)) ||
		(layer == "g" && player.s.best.gte(12))
	)
		return;
	if (
		(layer == "t" && player.h.best.gte(25)) ||
		(layer == "s" && player.q.best.gte(25)) ||
		(layer == "sb" && player.h.best.gte(2500)) ||
		(layer == "sg" && player.sg.best.gte(1))
	)
		return;
	if (
		(layer == "hb" && player.ba.best.gte(8)) ||
		(layer == "ss" && player.ba.best.gte(8))
	)
		return;
...
function buyUpg(layer, id) {
	...
	if (layer == "m" && id == 43)
		for (let i = 1; i <= 3; i++)
			player.m.spellTimes[i] *= LAYER_UPGS.m[43].currently().toNumber();
}
...
function addToBoosterBase() {
	...
	if (player.b.upgrades.includes(12))
		toAdd = toAdd.plus(LAYER_UPGS.b[12].currently());
	if (player.b.upgrades.includes(13))
		toAdd = toAdd.plus(LAYER_UPGS.b[13].currently());
	if (
		player.t.upgrades.includes(11) &&
		!(tmp.hcActive ? tmp.hcActive[12] : true)
	)
		toAdd = toAdd.plus(LAYER_UPGS.t[11].currently());
	if (
		player.t.upgrades.includes(31) &&
		!(tmp.hcActive ? tmp.hcActive[12] : true)
	)
		toAdd = toAdd.plus(25);
	if (
		player.t.upgrades.includes(33) &&
		!(tmp.hcActive ? tmp.hcActive[12] : true)
	)
		toAdd = toAdd.plus(40);
	...
	if (
		...
		toAdd = toAdd.plus(LAYER_UPGS.e[11].currently().b);
	if (player.s.unl && tmp.spaceBuildEff)
		toAdd = toAdd.plus(tmp.spaceBuildEff[2]);
	if (player.sb.upgrades.includes(21))
		toAdd = toAdd.plus(LAYER_UPGS.sb[21].currently());

	if (player.b.upgrades.includes(31))
		toAdd = toAdd.times(LAYER_UPGS.b[31].currently());
...
function getFreeBoosters() {
	...
	if (
		player.t.upgrades.includes(24) &&
		!(tmp.hcActive ? tmp.hcActive[12] : true)
	)
		free = free.plus(18);
	if (player.b.upgrades.includes(32))
		free = free.plus(LAYER_UPGS.b[32].currently());
...
function addToGenBase() {
	let toAdd = new Decimal(0);
	if (player.g.upgrades.includes(12))
		toAdd = toAdd.plus(LAYER_UPGS.g[12].currently());
	if (player.g.upgrades.includes(13))
		toAdd = toAdd.plus(LAYER_UPGS.g[13].currently());
	if (player.g.upgrades.includes(33))
		toAdd = toAdd.plus(LAYER_UPGS.g[33].currently());
	if (player.e.unl) toAdd = toAdd.plus(tmp.enhEff2);
	if (
		player.e.upgrades.includes(11) &&
		!(tmp.hcActive ? tmp.hcActive[12] : true)
	)
		toAdd = toAdd.plus(LAYER_UPGS.e[11].currently().g);
	if (player.s.unl && tmp.spaceBuildEff)
		toAdd = toAdd.plus(tmp.spaceBuildEff[2]);

	if (player.h.challs.includes(51))
		toAdd = toAdd.times(H_CHALLS[51].currently());
	if (player.q.upgrades.includes(53)) toAdd = toAdd.times(tmp.enhEff2);
	return toAdd;
}
...
function getGenPow() {
	...
	if (player.g.upgrades.includes(34))
		pow = pow.times(LAYER_UPGS.g[34].currently());
...
function getGenPowerGainMult() {
	let mult = new Decimal(1);
	if (player.g.upgrades.includes(21))
		mult = mult.times(LAYER_UPGS.g[21].currently());
	if (player.g.upgrades.includes(25))
		mult = mult.times(LAYER_UPGS.g[25].currently());
	if (
		player.e.upgrades.includes(35) &&
		!(tmp.hcActive ? tmp.hcActive[12] : true)
	)
		mult = mult.times(LAYER_UPGS.e[35].currently());
	if (
		player.s.upgrades.includes(12) &&
		!(tmp.hcActive ? tmp.hcActive[12] : true)
	)
		mult = mult.times(LAYER_UPGS.s[12].currently());
	if (
		player.s.upgrades.includes(13) &&
		!(tmp.hcActive ? tmp.hcActive[12] : true)
	)
		mult = mult.times(LAYER_UPGS.s[13].currently());
	if (player.q.unl && tmp.quirkEff) mult = mult.times(tmp.quirkEff);
	if (player.g.upgrades.includes(35))
		mult = mult.times(LAYER_UPGS.g[35].currently());
	return mult;
}
...
function getGenPowerEff() {
	if (tmp.hcActive ? tmp.hcActive[11] : true) return new Decimal(1);
	const eff = player.g.power.plus(1).pow(getGenPowerEffExp());
	return eff;
}
...
function getEnhancerPow() {
	...
	let pow = new Decimal(1);
	if (
		player.e.upgrades.includes(25) &&
		!(tmp.hcActive ? tmp.hcActive[12] : true)
	)
		pow = pow.times(LAYER_UPGS.e[25].currently());
	if (
		player.e.upgrades.includes(31) &&
		!(tmp.hcActive ? tmp.hcActive[12] : true)
	)
		pow = pow.times(LAYER_UPGS.e[31].currently());
	if (player.h.challs.includes(31)) pow = pow.times(2);
	if (player.q.upgrades.includes(42)) pow = pow.times(1.4);
	if (player.ba.upgrades.includes(32))
		pow = pow.times(LAYER_UPGS.ba[32].currently());
	return pow;
}

function getEnhancerEff() {
	if (!player.e.unl) return new Decimal(1);
	const e = player.e.enhancers.sub(tmp.subbedEnh).times(tmp.enhPow);
	let eff;
...
function getEnhancerCost() {
	...
	const cost = Decimal.pow(2, e.pow(1.5));
	return cost.floor();
}

function buyEnhancer() {
	const cost = getEnhancerCost();
...
function getCapPow() {
	...
	if (player.q.upgrades.includes(33))
		pow = pow.times(LAYER_UPGS.q[33].currently());
...
function getTimeEnergyEff() {
	...
	let exp = 1.2;
	if (
		player.t.upgrades.includes(21) &&
		!(tmp.hcActive ? tmp.hcActive[12] : true)
	)
		exp = 1.75;
	const eff = player.t.energy.plus(1).pow(exp);
	return eff;
}
...
function getTimeEnergyGainMult() {
	...
	if (
		...
		mult = mult.times(LAYER_UPGS.t[21].currently());
	if (
		player.t.upgrades.includes(22) &&
		!(tmp.hcActive ? tmp.hcActive[12] : true)
	)
		mult = mult.times(LAYER_UPGS.t[22].currently());
	if (
		player.t.upgrades.includes(23) &&
		!(tmp.hcActive ? tmp.hcActive[12] : true)
	)
		mult = mult.times(LAYER_UPGS.t[23].currently());
	if (
		player.t.upgrades.includes(34) &&
		!(tmp.hcActive ? tmp.hcActive[12] : true)
	)
		mult = mult.times(LAYER_UPGS.t[34].currently());
	return mult;
}
...
function getTimeEnergyLimitMult() {
	...
	if (
		...
		mult = mult.times(LAYER_UPGS.t[12].currently());
	if (
		player.t.upgrades.includes(21) &&
		!(tmp.hcActive ? tmp.hcActive[12] : true)
	)
	...
	if (player.q.upgrades.includes(24))
		mult = mult.times(LAYER_UPGS.q[24].currently());
...
function getExtCapsuleCost() {
	...
	if (amt.gte(25) && !player.ba.upgrades.includes(43))
		amt = amt.pow(2).div(25);
	const cost = amt.times(0.4).pow(1.2).plus(1).times(10);
	return cost.floor();
}

function buyExtCapsule() {
	if (!player.t.unl) return;
	const cost = getExtCapsuleCost();
...
function maxExtTimeCapsules() {
	...
	if (target.gte(25) && !player.ba.upgrades.includes(43))
		target = target.times(25).sqrt();
...
function addToTimeBase() {
	let toAdd = new Decimal(0);
	if (player.m.upgrades.includes(44))
		toAdd = toAdd.plus(LAYER_UPGS.m[44].currently());
	return toAdd;
}
...
function getSpace() {
	...
	if (
		...
		baseSpace = baseSpace.plus(2);
	if (
		player.s.upgrades.includes(24) &&
		!(tmp.hcActive ? tmp.hcActive[12] : true)
	)
		baseSpace = baseSpace.plus(3);
	if (player.ss.unl) baseSpace = baseSpace.plus(tmp.ssEff1);
	if (player.ss.upgrades.includes(11))
		baseSpace = baseSpace.plus(LAYER_UPGS.ss[11].currently());
...
function getSpaceBuildingCostMod() {
	...
	if (spellActive(4))
		mod = mod.times(Decimal.sub(1, tmp.spellEffs[4].sub(1)));
	return mod;
}
...
function getSpaceBuildingCostMult() {
	...
	if (player.s.upgrades.includes(41))
		mult = mult.div(LAYER_UPGS.s[41].currently());
	return mult;
}

function getSpaceBuildingCost(x) {
	const inputVal = new Decimal([1e3, 1e10, 1e25, 1e48, 1e100][x - 1]);
	let bought = player.s.buildings[x];
	if (bought.gte(100)) bought = bought.pow(2).div(100);
	const cost = Decimal.pow(
		inputVal,
		bought.times(getSpaceBuildingCostMod()).pow(1.35)
	)
		.times(inputVal)
		.times(bought.gt(0) || x > 1 ? 1 : 0)
		.times(getSpaceBuildingCostMult());
	return cost;
}

function getSpaceBuildingTarg(x) {
	const inputVal = new Decimal([1e3, 1e10, 1e25, 1e48, 1e100][x - 1]);
	let target = player.g.power
		.div(getSpaceBuildingCostMult())
		.div(inputVal)
		.max(1)
		.log(inputVal)
		.pow(1 / 1.35)
		.div(getSpaceBuildingCostMod());
...
function getSpaceBuildingPow() {
	...
	if (
		...
		pow = pow.times(LAYER_UPGS.s[21].currently());
	if (
		player.s.upgrades.includes(22) &&
		!(tmp.hcActive ? tmp.hcActive[12] : true)
	)
		pow = pow.times(LAYER_UPGS.s[22].currently());
	if (
		player.s.upgrades.includes(23) &&
		!(tmp.hcActive ? tmp.hcActive[12] : true)
	)
		pow = pow.times(LAYER_UPGS.s[23].currently());
	if (
		player.s.upgrades.includes(44) &&
		!(tmp.hcActive ? tmp.hcActive[12] : true)
	)
		pow = pow.times(1.25);
...
function getExtraBuildingLevels(x) {
	...
	if (
		...
		lvl = lvl.plus(1);
	if (
		player.s.upgrades.includes(14) &&
		!(tmp.hcActive ? tmp.hcActive[12] : true)
	)
		lvl = lvl.plus(1);
	if (player.q.upgrades.includes(31)) lvl = lvl.plus(1);
	if (x < 5) lvl = lvl.plus(tmp.spaceBuildEff[5]);
	if (player.m.upgrades.includes(32))
		lvl = lvl.plus(LAYER_UPGS.m[32].currently());
	return lvl;
}
...
function getSpaceBuildingEff(x) {
	...
	const power = getSpaceBuildingPow();
		...
		case 1:
			ret = Decimal.pow(
				Decimal.add(
					1,
					bought.pow(
						player.s.upgrades.includes(31) &&
							!(tmp.hcActive ? tmp.hcActive[12] : true)
							? 2.75
							: 1
					)
				),
				player.s.points.sqrt()
			)
				.times(
					Decimal.mul(
						4,
				...
				.max(1);
			if (player.ba.upgrades.includes(42))
				ret = ret.pow(LAYER_UPGS.ba[42].currently());
			return ret;
			break;
		case 2:
			if (player.s.upgrades.includes(43))
				return Decimal.pow(1.0001, bought).times(bought.sqrt());
			return bought.sqrt();
			break;
		case 3:
...
function getSpaceBuildingEffDesc(x) {
	const eff = tmp.spaceBuildEff[x];
	switch (x) {
		case 1:
			return (
				`Space Energy boosts Point gain & Prestige Point gain (${ 
				format(eff) 
				}x)`
			);
			break;
		case 2:
			return (
				`Adds to base of Booster/Generator effects by ${format(eff)}`
			);
			break;
		case 3:
		...
		case 4:
			return (
				`Booster Upgrade 6's effect is raised to the power of ${ 
				format(eff)}`
			);
			break;
		case 5:
			return (
				`Add ${ 
				format(eff) 
				} free levels to all previous Space Buildings.`
			);
			break;
	}
}
...
function buyBuilding(x) {
	...
	const cost = getSpaceBuildingCost(x);
...
function maxSpaceBuilding(x) {
	...
	const target = getSpaceBuildingTarg(x);
	const bulk = target.sub(player.s.buildings[x]).min(space);
	if (bulk.lt(1)) return;
...
function respecSpaceBuildings() {
	...
	if (
		!confirm(
			"Are you sure you want to reset your Space Buildings? This will force you to do a Space reset as well!"
		)
	)
		return;
...
function getSuperBoosterPow() {
	...
	let pow = new Decimal(1);
	if (
		player.sb.upgrades.includes(11) &&
		!(tmp.hcActive ? tmp.hcActive[12] : true)
	)
		pow = pow.times(LAYER_UPGS.sb[11].currently());
	if (
		player.sb.upgrades.includes(12) &&
		!(tmp.hcActive ? tmp.hcActive[12] : true)
	)
		pow = pow.times(LAYER_UPGS.sb[12].currently());
	if (player.hb.upgrades.includes(11))
		pow = pow.times(LAYER_UPGS.hb[11].currently());
	return pow;
}
...
function addToSBBase() {
	...
	if (player.sb.upgrades.includes(22))
		toAdd = toAdd.plus(LAYER_UPGS.sb[22].currently());
...
function getQuirkLayerCost() {
	...
	const cost = Decimal.pow(tmp.qCB, Decimal.pow(tmp.qCB, layers)).sub(1);
	return cost.max(1);
}
...
function getQuirkLayerMult() {
	...
	if (player.q.upgrades.includes(21))
		mult = mult.times(LAYER_UPGS.q[21].currently());
	if (player.q.upgrades.includes(52))
		mult = mult.times(LAYER_UPGS.q[52].currently());
	if (player.h.challs.includes(52))
		mult = mult.times(H_CHALLS[52].currently());
	if (player.ba.upgrades.includes(13))
		mult = mult.times(LAYER_UPGS.ba[13].currently());
	return mult;
}
...
function getExtraQuirkLayers() {
	...
	if (player.q.upgrades.includes(51))
		layers = layers.plus(LAYER_UPGS.q[51].currently());
	if (player.m.upgrades.includes(24))
		layers = layers.plus(LAYER_UPGS.m[24].currently());
...
function maxQuirkLayers() {
	...
	const cost = getQuirkLayerCost();
	if (player.q.points.lt(cost)) return;
	const target = getQuirkLayerTarg();
...
const H_CHALLS = {
	...
	12: {
		...
		unl() {
			return player.h.challs.includes(11);
		},
		...
		currently() {
			return Decimal.pow(
				1.5,
				player.q.layers.times(player.sp.upgrades.includes(22) ? 250 : 1)
			);
		},
		effDisp(x) {
			return `${format(x)}x`;
	...
	21: {
		...
		unl() {
			return player.h.challs.includes(12);
		},
		goal: new Decimal("1e1200"),
		reward:
			"Hindrance Spirit & Quirks make Time Capsules & Space Energy cheaper.",
		currently() {
			return player.h.points
				.plus(player.q.points)
				.div(2)
				.plus(1)
				.pow(1000)
				.pow(player.sp.upgrades.includes(22) ? 250 : 1);
		},
		effDisp(x) {
			return `${format(x)}x cheaper`;
		},
	},
	22: {
		name: "Impaired Nodes",
	...
	31: {
		...
		unl() {
			return player.h.challs.includes(21) || player.h.challs.includes(22);
		},
	...
	41: {
		name: "Skip the Third",
		desc:
			"Enhancers, Time Capsules, Space Buildings, and Super-Boosters do nothing.",
		unl() {
			return player.h.challs.includes(31) || player.h.challs.includes(32);
		},
	...
	42: {
		...
		desc:
			"Time slows down over time, halting to a stop after 10 seconds. Hint: This also impacts auto Enhance Point generation, so make sure to manually press E!",
		unl() {
			return player.h.challs.includes(31) && player.h.challs.includes(32);
		},
	...
	51: {
		name: "It's all Gone",
		desc:
			'"Skip the Second" and "Skip the Third" are both applied at once.',
		unl() {
			return player.h.challs.includes(41) && player.h.challs.includes(42);
		},
		goal: new Decimal("1e2840"),
		reward: "Super-Boosters multiply the Generator base.",
		currently() {
			return player.sb.points.plus(1).sqrt();
		},
		effDisp(x) {
			return `${format(x)}x`;
		},
	...
	52: {
		name: "Anti-Enhancers",
		desc:
			"You lose Enhancers over time, which can make your Enhancer amount get below 0. Hint: Maybe it's best to not have any Time Capsules or Space Energy?",
		unl() {
			return (
				player.h.challs.includes(41) &&
				player.h.challs.includes(42) &&
				player.h.challs.includes(51)
			);
		},
		goal: new Decimal("1e440000"),
		reward:
			"Quirk Layers are faster based on your Hindrance Spirit & Quirks.",
		currently() {
			...
			if (h.gte(1e150))
				h = h
					.log10()
					.pow(50)
					.times(1e150 / Math.pow(150, 50))
					.min(h);
			if (h.gte(1e100)) h = h.times(1e100).sqrt();
			const ret = h.plus(1).pow(0.04);
			return ret;
		},
		effDisp(x) {
			return `${format(x)}x`;
	...
	61: {
		name: "Microanalysis",
		desc:
			'"Flattened Curve" and "Surprised Junction" are both applied at once.',
		unl() {
			return player.m.upgrades.includes(12);
		},
	...
	71: {
		...
		desc:
			'All previous Hindrances (except "Slowed to a Halt" and "Anti-Enhancers") are applied at once.',
		unl() {
			return player.m.upgrades.includes(31);
		},
		...
		currently() {
			...
			if (ret.gte("1.8e308"))
				ret = ret.sqrt().times(Decimal.sqrt("1.8e308"));
			return ret;
		},
		effDisp(x) {
			return `${format(x)}x`;
...
function startHindrance(x) {
	...
	if (player.h.active == x) {
		if (
			player.points.gte(H_CHALLS[x].goal) &&
			!player.h.challs.includes(x)
		) {
...
function getSubspaceEff1() {
	...
	const eff = player.ss.subspace
		.times(player.ss.points)
		.plus(1)
		.log10()
		.times(100);
	return eff.floor();
...
function getSubspaceEff2() {
	...
	const eff = player.ss.subspace.plus(1).pow(750);
	return eff;
}

function getSubspaceEff3() {
	if (!player.ss.unl) return new Decimal(1);
	let eff = player.ss.subspace
		.plus(1)
		.log10()
		.plus(1)
		.log10()
...
function getSubspaceGainMult() {
	let mult = new Decimal(1);
	if (player.ss.upgrades.includes(12))
		mult = mult.times(LAYER_UPGS.ss[12].currently());
	if (player.ss.upgrades.includes(22))
		mult = mult.times(LAYER_UPGS.ss[22].currently());
	if (player.ss.upgrades.includes(24))
		mult = mult.times(LAYER_UPGS.ss[24].currently());
	if (player.ss.upgrades.includes(25))
		mult = mult.times(LAYER_UPGS.ss[25].currently());
	if (player.ba.upgrades.includes(12))
		mult = mult.times(LAYER_UPGS.ba[12].currently());
	return mult;
}

function addToHBBase() {
	let toAdd = new Decimal(0);
	if (player.m.upgrades.includes(13))
		toAdd = toAdd.plus(LAYER_UPGS.m[13].currently());
	if (player.m.upgrades.includes(33))
		toAdd = toAdd.plus(LAYER_UPGS.m[33].currently());
	return toAdd;
}
...
function getHyperBoosterPow() {
	let pow = new Decimal(1);
	if (player.hb.upgrades.includes(12))
		pow = pow.times(LAYER_UPGS.hb[12].currently());
	return pow;
}
...
function getBalanceTypesEff() {
	let mod = player.ba.positivity
		.plus(1)
		.log10()
		.plus(1)
		.div(player.ba.negativity.plus(1).log10().plus(1))
		.log10()
		.abs()
		.plus(1)
		.pow(-1);
	const pos = player.ba.positivity.plus(1).log10().plus(1);
	const neg = player.ba.negativity.plus(1).log10().plus(1);
	if (player.ba.upgrades.includes(34)) mod = mod.times(1.5);
	if (player.ba.upgrades.includes(53))
		mod = mod.times(LAYER_UPGS.ba[53].currently());
	const eff = pos.times(neg).pow(mod);
	return eff;
}
...
function getPosGainMult() {
	let mult = new Decimal(1);
	if (player.ba.upgrades.includes(22))
		mult = mult.times(LAYER_UPGS.ba[22].currently());
	return mult;
}
...
function getBalPowGainMult() {
	let mult = new Decimal(1);
	if (player.sp.upgrades.includes(14))
		mult = mult.times(LAYER_UPGS.sp[14].currently());
	return mult;
}
...
function getSpellPower(x) {
	...
	if (player.m.upgrades.includes(11))
		power = power.times(LAYER_UPGS.m[11].currently());
	if (player.m.upgrades.includes(21) && (x == 2 || x == 3))
		power = power.times(LAYER_UPGS.m[21].currently());
	if (player.m.upgrades.includes(22) && x == 2) power = power.times(10);
	if (player.m.upgrades.includes(41)) {
		const casted = player.m.casted[x];
		power = power.times(
			casted.max(1).log10().plus(1).log10().div(5).plus(1)
		);
	}
	if (player.sp.upgrades.includes(23))
		power = power.times(LAYER_UPGS.sp[23].currently());

	if (power.gte(50))
		power = power
			.log10()
			.times(50 / Math.log10(50))
			.min(power);
	return power.max(1);
}

function getSpellEff(x) {
	const base = SPELL_BASE[x];
	const power = getSpellPower(x);
...
function getSpellDesc(x) {
	const desc = SPELL_DESCS[x];
	const eff = tmp.spellEffs[x];
...
function getSpellTime() {
	...
	if (player.m.upgrades.includes(43))
		time *= LAYER_UPGS.m[43].currently().toNumber();
	return time;
}
...
function activateSpell(x, force = false) {
	...
	const toCast = setToCast(player.m.toCast[x]).max(1);
	if (!force) {
		if (!player.m.unl) return;
		if (spellActive(x)) return;
		if (player.m.points.lt(toCast)) return;
	}
	if (player.sp.total.lt(2))
		player.m.points = player.m.points.sub(toCast).max(0);
...
function getHexGain() {
	let gain = new Decimal(1);
	if (player.m.upgrades.includes(14))
		gain = gain.times(LAYER_UPGS.m[14].currently());
	if (player.sp.upgrades.includes(14))
		gain = gain.times(LAYER_UPGS.sp[14].currently());
	return gain.floor();
}

function getHexEff() {
	const eff = player.m.hexes.plus(1).pow(5);
	return eff;
}
...
function setToCast(val) {
	...
	const validVal = isToCastValid(val);
...
function getSGenPowEff() {
	...
	const eff = player.sg.power.plus(1).pow(3);
	return eff;
}

function getSuperGenPowerGainMult() {
	let mult = new Decimal(1);
	if (player.ba.upgrades.includes(21))
		mult = mult.times(LAYER_UPGS.ba[21].currently());
	return mult;
}

function addToSGBase() {
	let toAdd = new Decimal(0);
	if (player.ba.upgrades.includes(23))
		toAdd = toAdd.plus(LAYER_UPGS.ba[23].currently());
	if (player.sp.upgrades.includes(21))
		toAdd = toAdd.plus(LAYER_UPGS.sp[21].currently());
	return toAdd;
}
...
function gameLoop(diff) {
	...
	if (tmp.hcActive ? tmp.hcActive[42] : true) {
		if (player.h.time >= 10) diff = new Decimal(0);
			...
			diff = diff.div(
				Decimal.div(10, Decimal.sub(10, player.h.time + 1)).pow(1000)
			);
	}
	...
	if (player.g.unl)
		player.g.power = player.g.power
			.plus(tmp.layerEffs.g.times(diff))
			.max(0);
	if (player.g.best.gte(10)) {
		player.p.points = player.p.points
			.plus(tmp.resetGain.p.times(diff))
			.max(0);
		player.p.best = player.p.best.max(player.p.points);
	}
	if (player.t.unl) {
		const data = tmp.layerEffs.t;
		player.t.energy = player.t.energy
			.plus(data.gain.times(diff))
			.min(data.limit)
			.max(0);
	}
	if (player.sg.unl)
		player.sg.power = player.sg.power
			.plus(tmp.layerEffs.sg.times(diff))
			.max(0);
	if (player.q.unl) {
		const mult = getQuirkLayerMult();
		player.q.time = player.q.time.plus(mult.times(diff)).max(0);
		const exp = getQuirkEnergyGainExp();
		if (exp.gte(0))
			player.q.energy = player.q.energy
				.plus(player.q.time.pow(exp).times(mult).times(diff))
				.max(0);
	}
	if (player.q.best.gte(15)) {
		player.e.points = player.e.points
			.plus(tmp.resetGain.e.times(diff))
			.max(0);
		player.e.best = player.e.best.max(player.e.points);
	}
	if (player.ss.unl)
		player.ss.subspace = player.ss.subspace
			.plus(tmp.layerEffs.ss.times(diff))
			.max(0);
	if (player.ba.unl) {
		player.ba.power = player.ba.power
			.plus(
				tmp.layerEffs.ba.power
					.times(tmp.balEff2)
					.times(getBalPowGainMult())
					.times(diff)
			)
			.max(0);
		player.ba.positivity = player.ba.positivity
			.plus(tmp.layerEffs.ba.pos.times(getPosGainMult()).times(diff))
			.max(0);
		player.ba.negativity = player.ba.negativity
			.plus(tmp.layerEffs.ba.neg.times(getNegGainMult()).times(diff))
			.max(0);
	}
	if (player.m.unl)
		for (let i = 1; i <= tmp.spellsUnl; i++)
			player.m.spellTimes[i] = Decimal.sub(player.m.spellTimes[i], diff)
				.max(0)
				.toNumber();
	if (player.m.best.gte(3)) {
		player.h.points = player.h.points
			.plus(tmp.resetGain.h.times(diff))
			.max(0);
		player.q.points = player.q.points
			.plus(tmp.resetGain.q.times(diff))
			.max(0);
		player.h.best = player.h.best.max(player.h.points);
		player.q.best = player.q.best.max(player.q.points);
	}
	if (player.m.best.gte(2.5e9))
		player.m.hexes = player.m.hexes.plus(getHexGain().times(diff)).max(0);
	if (player.sp.total.gte(10)) {
		player.m.points = player.m.points
			.plus(tmp.resetGain.m.times(diff))
			.max(0);
		player.ba.points = player.ba.points
			.plus(tmp.resetGain.ba.times(diff))
			.max(0);
	...
	if (player.s.autoBuild && player.ss.best.gte(1))
		for (let i = tmp.sbUnl; i >= 1; i--) maxSpaceBuilding(i);
	...
	if (player.m.autoIns && player.sp.total.gte(2))
		for (let i = 1; i <= tmp.spellsUnl; i++) {
			player.m.casted[i] = player.m.points;
			player.m.toCast[i] = player.m.points;
		}
	if (player.m.auto && player.m.best.gte(1000))
		for (let i = 1; i <= tmp.spellsUnl; i++) activateSpell(i);
	...
	if (!LAYERS.includes(key) || ctrlDown || shiftDown) {
		switch (key) {
			...
			case "3":
				...
				break;
			case "4":
				if (player.m.unl && player.sp.upgrades.includes(13))
					activateSpell(4);
				break;
			case "P":
